...
const MobileNavbar = (props: MobileNavbarProps) => {
  ...
  return (
    <div className='z-20'>
      <div>
        <button
          className={`rounded-full p-3 text-white transition-all active:scale-95 bg-black ${!isOpen && 'shadow-xl'} ${
            isOpen && 'text-white'
          }`}
      ...
      >
        {props.items.map((e, i) => (
          <MobileNavbarItem onClick={() => setIsOpen(false)} key={i} href={e.href} text={e.text} alternatives={e.alternatives} />
        ))}
      </nav>
    </div>
  );
